...
# JSON APIs to view League Information
...
def editLeague(league_id):
    ...
    if editedLeague.user_id != login_session['user_id']:
        return "<script>function myFunction()" \
               "{alert('You are not authorized to edit this soccer league. " \
...
def deleteLeague(league_id):
    ...
    if deletedLeague.user_id != login_session['user_id']:
        return "<script>function myFunction()" \
               "{alert('You are not authorized to delete this soccer league." \
               " Please create your own league in order to delete.');}" \
               "</script><body onload='myFunction()''>"
    if request.method == 'POST':
...
def newTeam(league_id):
    ...
    if login_session['user_id'] != league.user_id:
        return "<script>function myFunction()" \
               "{alert('You are not authorized to add team to this " \
               "league. Please create your own league in order to add" \
               " teams.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        newItem = Club(name=request.form['name'],
...
# edit team
@app.route('/leagues/<int:league_id>/league/<int:team_id>/edit/',
           methods=['GET', 'POST'])
@login_required
def editTeam(league_id, team_id):
    ...
    if login_session['user_id'] != league.user_id:
        return "<script>function myFunction()" \
               "{alert('You are not authorized to edit team to this " \
               "league. Please create your own league in order to edit" \
               " teams.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
...
# delete team
@app.route('/leagues/<int:league_id>/league/<int:team_id>/delete',
           methods=['GET', 'POST'])
@login_required
def deleteTeam(league_id, team_id):
    ...
    if login_session['user_id'] != league.user_id:
        return "<script>function myFunction()" \
               "{alert('You are not authorized to delete team from this" \
               "league. Please create your own league in order to delete" \
               " teams.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
